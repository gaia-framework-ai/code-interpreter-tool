...
async function processCellOutputData(data: string | string[], outputDir: string): Promise<string | string[]> {
  if (typeof data === 'string') {
    ...
    // Here, we upload the file to blob storage and return the blob URL
    // e.g., return await someService.uploadFileToBlob(filePath);
  ...
  if (Array.isArray(data)) {
    return Promise.all(
      data.map(async (item) => {
        ...
        // Here, we would upload the file to blob storage and return the blob URL
        // e.g., return await someService.uploadFileToBlob(filePath);
...
async function extractImageUrls(notebook: JupyterNotebook, outputDir: string): Promise<string[]> {
  ...
  return await Promise.all(base64Images.map(async (base64Image, index) => {
    // Here, we would upload the image to blob storage and return the blob URL
    // e.g., return await someService.uploadBase64ImageToBlob(base64Image, BlobType.Image);
